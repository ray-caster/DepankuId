...
@opportunity_bp.route('/<opportunity_id>/applications/<application_id>/status', methods=['PUT'])
@require_auth
def update_application_status(user_id: str, user_email: str, opportunity_id: str, application_id: str):
    ...
    try:
        ...
        # Verify that the user owns this opportunity
        opportunity = OpportunityService.get_opportunity_by_id(opportunity_id)
        if not opportunity:
            return jsonify({
                "success": False,
        ...
        if opportunity.get('created_by_uid') != user_id:
            return jsonify({
                "success": False,
                "error": "You don't have permission to update applications for this opportunity"
            }), 403
